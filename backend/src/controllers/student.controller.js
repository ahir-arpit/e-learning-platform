    ...
    try {
        ...
        const mailOptions = {
            ...
            <div style="text-align: center;">
                ...
                <br>
                <a href="http://localhost:5001/api/student/verify?id=${createdStudent_id}">
        ...
        console.log("---------------------------------------------------");
        console.log("⚠️ EMAIL NOT CONFIGURED? VERIFY MANUALLY HERE:");
        console.log(`http://localhost:5001/api/student/verify?id=${createdStudent_id}`);
        console.log("---------------------------------------------------");

        emailsender.sendMail(mailOptions, function (error) {
            if (error) {
                // Don't throw error if email fails, just log it so user can still sign up
                console.log("Email failed to send (missing credentials), but user created.");
                console.log("Check console above for manual verification link.");
